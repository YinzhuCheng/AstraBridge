...
| Durable run state and scheduler events | `astrabridge_sidecar.durable_run_store.DurableRunEventStore` + `astrabridge_sidecar.graph_scheduler.DurableGraphScheduler` | Workspace-local `.astrabridge/durable_runs.sqlite3` is the source of truth; `queue_task_graph_run` returns a durable receipt, `graph_run_status` reads the projection/events, and task JSON, manifests, diagnostics, and UI refs are redacted compatibility projections |
