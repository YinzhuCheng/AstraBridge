...
| --- | --- | --- | --- |
| **Local project and task workspace:** AstraBridge uses `.abproj` plus workspace-local `.astrabridge/`, and presents `Project -> Task` rather than raw runtime threads. | `current-contract` | [Architecture](ARCHITECTURE.md) owns the Desktop/sidecar/runtime boundary; [security and isolation guidance](SECURITY_AND_ISOLATION.md) defines isolated state; the project summary defines task/lane vocabulary. | Step 3's clean-clone fixture revision `c8988fef6f1139ac056fadb68e395122ee59254a` is the local canonical evidence revision. It is source-clone evidence only; do not claim a release installer or broad public fresh-install experience yet. |
| **Codex-derived core integration:** AstraBridge is built around Codex CLI/app-server runtime patterns, with AstraBridge-owned isolation and policy boundaries. | `current-contract` | [Architecture](ARCHITECTURE.md), [core multi-provider execution contract](/D:/AstraBridge/PLAN/CODEX_CORE_MULTI_PROVIDER_EXECUTION_CONTRACT.md), and current security guidance. | It is not the official Codex App, does not use official account login as a product path, and does not promise feature parity with any external Codex product. |
| **Evidence-qualified external provider lanes:** provider, model, endpoint, driver, authority, and evidence are separate decisions. | `current-contract` | [Provider capability and reasoning runbook](PROVIDER_MODEL_COMPATIBILITY_RUNBOOK.md) and the core execution contract assign the owner surfaces and promotion lifecycle. | A compatible HTTP response or metadata record cannot be marketed as coding-agent compatibility. |
...
| **High-freedom orchestration through code and GUI-facing graph contracts:** the bounded native Code Fix / Test / Review graph can be imported, rendered, dry-run, fixture-run, exported, re-imported, and compared without semantic change. | `deterministic-evidence` | [GUI / Code Orchestration Parity](GUI_CODE_ORCHESTRATION_PARITY.md), `tests.test_gui_code_orchestration_parity`, and its secret-free evidence prove 4 nodes, 3 edges, the `filesystem_write_gate`/`ask` boundary, source-owned direct-edit blocking, a passing dry-run, completed fixture run, and a `no_change` native round trip with no provider or network calls. | This is not a blanket promise of lossless conversion for every graph or external format. A source-owned graph never writes back; explicit detach creates a separate GUI copy, and any unsupported transform must report loss or fail safely. |
| **Flagship no-provider coding-agent reference:** developers can inspect a bounded code-fix/test/review workflow with task state, an approval-gated write boundary, artifacts, an injected failure, and retry recovery. | `deterministic-evidence` | [Flagship Coding-Agent Reference](FLAGSHIP_CODING_AGENT_REFERENCE.md), its scenario manifest, and `tests.test_flagship_coding_agent_reference` own the local no-key proof. The manifest declares route metadata only; the runner records zero provider calls. | This does not prove a live provider response, authorized coding-route eligibility, autonomous write authority, tool use, or release-installation behavior. |
...
| **No-key evaluation path.** | `deterministic-evidence` | [No-Key First Ten Minutes](NO_KEY_FIRST_TEN_MINUTES.md) records an isolated Windows clean clone of local canonical revision `c8988fef6f1139ac056fadb68e395122ee59254a`: fresh application/runtime/Codex roots, a `.abproj` project, a `Supervisor / Worker / Synthesizer` fixture graph, and a completed run with 3 workers and 22 artifacts. `npm.cmd ci`, focused task-graph tests, build, editable sidecar install, import, and `pip check` passed; browser requests were loopback-only. | This proves exact-source no-provider fixture behavior only. It does not prove a release installer, live provider/model behavior, tool authority, coding-route eligibility, or a published distribution. |
| **Extensions are classified rather than implicitly trusted:** the documented native graph subset is supported; candidate skills, plugin/preset flows, metadata updates, and external adapters retain explicit experimental boundaries; runtime/transport internals and public marketplace/intake remain internal or deferred. | `current-contract` plus `deterministic-evidence` | [Extension and First-Contribution Surface](EXTENSION_AND_FIRST_CONTRIBUTION_SURFACE.md) owns the classification. Its candidate `example.contributor-read-only-brief` resolves to an existing graph and passes provider-free lint/compile/dry-run while an out-of-allowlist route request is blocked. | Do not market a plugin marketplace, automatic skill execution, a live provider extension, or external merge-ready intake. Candidate validation does not grant enablement, install, tool/write authority, or route qualification. |
| **Prepared pre-preview feedback and first-contribution protocol.** | `deterministic-evidence; public intake pending` | [Contributor Feedback Protocol](CONTRIBUTOR_FEEDBACK_PROTOCOL.md), five local GitHub-compatible templates, and the provider-free cohort runner validate three bounded candidates and two independent candidate-skill rehearsals while retaining an authority-widening block. | This does not activate an issue tracker, private reporting route, named maintainer, support SLA, license, contribution terms, or merge-ready code intake. Public intake may be enabled only after the protocol's explicit foundation and maintainer conditions are satisfied. |
| **Developer-preview evaluation baseline and readiness decision.** | `deterministic-evidence; public preview paused` | [Public Quality and Reliability Dossier](PUBLIC_QUALITY_RELIABILITY_DOSSIER.md) is the failure-first index across the seven current evidence cards; [Developer Preview Baseline](DEVELOPER_PREVIEW_BASELINE.md) owns the bounded source route, release-identity/staging evidence, isolated Windows update rehearsal, report contents, and named owners; [Developer Preview Readiness Decision](DEVELOPER_PREVIEW_READINESS_DECISION.md) applies DG-OSS-04 and records mandatory branch C `pause`; [foundation decision record](OPEN_SOURCE_FOUNDATION_DECISION_RECORD.md), [CONTRIBUTING](../CONTRIBUTING.md), [CODE_OF_CONDUCT](../CODE_OF_CONDUCT.md), and [SECURITY](../SECURITY.md) own the remaining foundation gates. | It is accurate to describe the no-key source route and deterministic package/update rehearsal. No public installer, signed update service, supported-version, private reporting/contact, license, contribution-intake, or distribution claim is valid until every named owner gate has current evidence. |
