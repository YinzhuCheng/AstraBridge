            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "source_urls": {
                        ...
                        "maxItems": 10,
                        "items": {"type": "string"},
                        "description": "Optional known public URLs to fetch. If source_urls are provided without explicit queries, live search expansion is skipped to avoid mixing unrelated sources.",
                    },
...
def _search_batch(queries: Any, *, dedupe: bool, timeout_sec: int) -> dict[str, Any]:
    ...
    for spec in query_specs:
        query = spec["query"]
        raw_results: list[dict[str, str]] = _hint_search_results_for_query(query)
...
def _hint_search_results_for_query(query: str) -> list[dict[str, str]]:
    lowered = str(query or "").lower()
    if "openai" not in lowered or "structured output" not in lowered:
        return []
    return [
        {
            "title": "OpenAI structured outputs guide",
            "url": "https://platform.openai.com/docs/guides/structured-outputs",
            "snippet": "Hinted official source for OpenAI structured outputs documentation.",
            "query_variant": "hinted_official_source",
        },
        {
            "title": "OpenAI Responses API text guide",
            "url": "https://platform.openai.com/docs/guides/text?api-mode=responses",
            "snippet": "Hinted official source for Responses API text and structured output usage.",
            "query_variant": "hinted_official_source",
        },
        {
            "title": "OpenAI Responses API reference",
            "url": "https://platform.openai.com/docs/api-reference/responses/create",
            "snippet": "Hinted official API reference page for creating Responses API calls.",
            "query_variant": "hinted_official_source",
        },
    ]
...
def _research_brief(
    *,
    ...
    explicit_source_urls = _string_list(source_urls)[:10]
    pinned_sources_only = bool(explicit_source_urls) and not explicit_queries
    query_texts = explicit_queries or ([] if pinned_sources_only else _derive_research_queries(research_goal))
    hinted_urls = [] if explicit_source_urls else _hint_source_urls_for_goal(research_goal)
    if pinned_sources_only:
        search_payload = {
            "query_count": 0,
            "result_count": 0,
            "warnings": ["Pinned source_urls were supplied; live search expansion was skipped to avoid mixing unrelated query results."],
            "merged_results": [],
        }
        source_policy = {
            "mode": "pinned_source_urls",
            "pinned_source_count": len(explicit_source_urls),
            "hinted_source_count": 0,
            "search_expansion": "skipped",
            "reason": "source_urls supplied without explicit queries",
        }
    elif not explicit_queries and hinted_urls:
        search_payload = {
            "query_count": 0,
            "result_count": 0,
            "warnings": ["Used hinted public first-party sources; live search expansion was skipped to keep deep research responsive."],
            "merged_results": [],
        }
        source_policy = {
            "mode": "hinted_first_party_sources",
            "pinned_source_count": 0,
            "hinted_source_count": len(hinted_urls),
            "search_expansion": "skipped",
            "reason": "first-party source hints matched the research goal",
        }
    else:
        ...
        source_policy = {
            ...
            "reason": "explicit queries were supplied" if explicit_queries else "derived query plan used",
        }
    candidates: list[dict[str, str]] = []
    for url in explicit_source_urls:
        candidates.append({"title": "", "url": url, "snippet": "User-provided source URL.", "query": "source_urls", "source_origin": "pinned_source_url"})
    for url in hinted_urls:
        candidates.append({"title": "", "url": url, "snippet": "AstraBridge hinted technical source.", "query": "hinted_sources", "source_origin": "hinted_source"})
    for item in list(search_payload.get("merged_results") or []):
        if isinstance(item, dict):
            search_item = dict(item)
            search_item.setdefault("source_origin", "search_result")
            candidates.append(search_item)
    ...
    for candidate in candidates[:fetch_top_n]:
        ...
        source = {
            ...
            "source_origin": str(candidate.get("source_origin") or "search_result"),
            ...
            source.update(
                {
                    ...
                    "source_host": str(fetched.get("source_host") or _source_host(url)),
                    ...
                    "access_date": str(fetched.get("access_date") or ""),
    ...
    cached_sources = [item for item in fetched_sources if item.get("cache_hit")]
    unresolved = []
    ...
    return {
        ...
        "source_policy": {
            **source_policy,
            "search_result_count": search_payload.get("result_count"),
        },
        "fetch_summary": {
            "requested_count": len(sources),
            "ok_count": len(fetched_sources),
            "failed_count": len(failures),
            "cache_hit_count": len(cached_sources),
        },
        ...
        "citation_rule": "Use only URLs in sources for citations, and include access_date when freshness matters. Mark unfetched search snippets as unverified.",
    }
...
def _hint_source_urls_for_goal(goal: str) -> list[str]:
    ...
    urls: list[str] = []
    if "openai" in lowered and "structured output" in lowered:
        urls.extend(
            [
    ...
    if "openai" in lowered and "structured output" in lowered:
        variants.extend(
            [
                ...
                "site:platform.openai.com/docs/guides structured outputs Responses API",
                "site:developers.openai.com/api Responses API structured outputs",
            ]
        )
...
def _score_result_for_query(
    ...
    *,
    ...
    if domain in preferred:
        score += 5.0
    if any(domain.endswith(item) for item in ("platform.openai.com", "developers.openai.com", "openai.com")) and "openai" in query.lower():
        score += 4.0
        if any(token in text for token in ("structured outputs", "responses api", "json schema")):
            score += 4.0
...
def _access_date(timestamp: str) -> str:
...
def _fetch_cache_lookup(url: str) -> dict[str, Any] | None:
    key = _canonical_url(url)
...
def _fetch_cache_store(url: str, payload: dict[str, Any]) -> None:
    key = _canonical_url(url)
    _FETCH_CACHE[key] = dict(payload)
    _FETCH_CACHE.move_to_end(key)
    while len(_FETCH_CACHE) > FETCH_CACHE_MAX_ENTRIES:
        _FETCH_CACHE.popitem(last=False)


def _derive_research_queries(goal: str) -> list[str]:
    compact = re.sub(r"\s+", " ", goal).strip()
    search_subject = _research_search_subject(compact)
    queries = [search_subject]
    lowered = compact.lower()
    subject_lowered = search_subject.lower()
    if "openai" in subject_lowered and "structured output" in subject_lowered:
        queries.extend(
            [
                "site:platform.openai.com/docs OpenAI Responses API structured outputs",
                "site:platform.openai.com/docs/guides structured outputs Responses API",
                "site:developers.openai.com/api Responses API structured output",
            ]
        )
    ...
    if any(token in lowered for token in ["\u9b54\u5854", "tower", "rpg", "game", "autotile", "sprite"]):
        queries.extend(
            [
                f"{search_subject} best practices",
                f"{search_subject} implementation guide",
                f"{search_subject} examples",
                "RPG autotile rules tilemap edge corner transition",
                "magic tower game design deterministic resource planning",
            ]
        )
    else:
        queries.extend([f"{search_subject} official documentation", f"{search_subject} guide", f"{search_subject} best practices"])
    return list(dict.fromkeys(query[:300] for query in queries if query))[:DEFAULT_BATCH_MAX_QUERIES]


def _research_search_subject(goal: str) -> str:
    compact = re.sub(r"\s+", " ", goal).strip()
    lowered = compact.lower()
    if "openai" in lowered and "responses api" in lowered and "structured output" in lowered:
        return "OpenAI Responses API structured outputs official documentation"
    without_instruction = re.sub(
        r"^(summarize|compare|explain|research|investigate|find|review)\s+",
        "",
        compact,
        flags=re.IGNORECASE,
    )
    without_source_clause = re.sub(
        r",?\s*(citing|using|from)\s+official\s+source\s+pages\s+only\.?$",
        "",
        without_instruction,
        flags=re.IGNORECASE,
    )
    return without_source_clause.strip(" .") or compact
...
def _fetch(url: str, *, max_chars: int, timeout_sec: int) -> dict[str, Any]:
    ...
    cached = _fetch_cache_lookup(url)
    if cached:
        ...
        return {
            ...
            "source_host": str(cached.get("source_host") or _source_host(str(cached.get("url") or url))),
            "content_type": str(cached.get("content_type") or "text/plain"),
            "text": text[:max_chars],
            "truncated": truncated,
            "char_count": int(cached.get("char_count") or len(text)),
            "cache_hit": True,
            "fetched_at": str(cached.get("fetched_at") or ""),
            "access_date": str(cached.get("access_date") or ""),
            "status_code": cached.get("status_code"),
        }
    body, final_url, content_type, status_code = _download_text(url, timeout_sec=timeout_sec)
    ...
    payload = {
        ...
        "access_date": _access_date(fetched_at),
        "status_code": status_code,
    }
    _fetch_cache_store(final_url, payload)
    return {
        **payload,
        "text": text[:max_chars],
        "truncated": truncated,
...
def _download_text(url: str, *, timeout_sec: int) -> tuple[str, str, str, int]:
    _validate_public_url(url)
    ...
    with urllib.request.urlopen(request, timeout=max(5, min(60, int(timeout_sec or DEFAULT_TIMEOUT_SEC)))) as response:  # noqa: S310
        ...
        return text, str(response.geturl() or url), content_type, int(getattr(response, "status", 200) or 200)
