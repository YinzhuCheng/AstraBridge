...
Those overlapping concerns are now owned by `PLAN/ASTRABRIDGE_PRODUCT_STABILITY_AND_INTEROPERABILITY_EXECUTION_PLAN.md`, which is the single active execution source for shared stability, external A2A, orchestration, update, and release work. The completed `PLAN/ASTRABRIDGE_STABILITY_PROTOCOL_AND_AGENT_RUNTIME_EXECUTION_PLAN.md` remains the implementation baseline. Capability-specific work may continue only when it does not create a second MCP, envelope, artifact, scheduler, run-state, A2A gateway, executor, or updater contract. This is a routing clarification, not a reset of the completed capability step or its preserved evidence.
